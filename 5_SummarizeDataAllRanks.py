...
TaxLst = []
LineageDict = {}
LineageDict['aaa_header'] = ['k','p','c','o','f','g','s']
				...
				if len(tax.split(';')) >= (int(args.Rank)+1):
					if tax.split(';')[int(args.Rank)] not in TaxLst:
						if tax.split(';')[int(args.Rank)] != '':
							...
							LineageDict[tax.split(';')[int(args.Rank)]] = tax.split(';')
...
for i in range(8):
	for key in LineageDict:
		if len(LineageDict[key]) < 7:
			LineageDict[key].append('')
...
with open(args.OutFileName, 'w') as OutFile:
	for key, value in sorted(TaxDct.items()):
		L = LineageDict[key]
		L = L[0:(int(args.Rank)+1)]
		L = ','.join(L)
		OutFile.write(L + ',' + str(key) + ',' +  ','.join(value) + '\n')
