...
#### Raw data and reference databases used can be found at [Link coming soon]
...
do
        ...
        vsearch --usearch_global ${a}.fa --db 28S_Amplicons.fasta --id 0.70 \
        ...
        python ./../3_VsearchToMetaxa2.py -v ${a}.vsrch70pi.txt -t 28S_Amplicons.tax -o ${a}.mtxa2.tax
...
do
        ...
        # calculate alignments
        vsearch --usearch_global ${a}_MrgClp.fa --db Updated_CO1_Amplicons.fasta --id 0.70 \
                --maxaccepts 100 --maxrejects 50 --maxhits 1 --gapopen 0TE \
                --gapext 0TE --userout ${a}.vsrch70pi.txt --userfields query+target+id+alnlen+mism+opens+qlo+qhi+tlo+thi+evalue+bits+qcov \
                --query_cov 0.8 --threads 28
        # annotate vsearch tabular output with taxonomic lineages
        python ./../3_VsearchToMetaxa2.py -v ${a}.vsrch70pi.txt -t Updated_CO1_Amplicons.tax -o ${a}.mtxa2.tax
        # trim annotations based on percent ID match
        python ./../4_TrimMtxa2IDs.py ${a}.mtxa2.tax ${a}.Trmd.txt
done
...
do
        ...
        # calculate alignments
        vsearch --usearch_global ${a}_MrgClp.fa --db Updated_16S_Amplicons.fasta --id 0.70 \
                --maxaccepts 100 --maxrejects 50 --maxhits 1 --gapopen 0TE \
                --gapext 0TE --userout ${a}.vsrch70pi.txt --userfields query+target+id+alnlen+mism+opens+qlo+qhi+tlo+thi+evalue+bits+qcov \
                --query_cov 0.8 --threads 28 --strand both
        # annotate vsearch tabular output with taxonomic lineages
        python ./../3_VsearchToMetaxa2.py -v ${a}.vsrch70pi.txt -t Updated_16S_Amplicons.tax -o ${a}.mtxa2.tax
        # trim annotations based on percent ID match
        python ./../4_TrimMtxa2IDs.py ${a}.mtxa2.tax ${a}.Trmd.txt
done
